...
def add_to_x(opcode, cpu):
    ...
    cpu.V[opcode.x] &= 0xFF  # restrict the value to one byte or less
 
def add_x_to_i(opcode, cpu):
    cpu.clear_arithmetic_flag()
    original_value = cpu.I
    result = cpu.I + cpu.V[opcode.x]
...
def add_y_to_x(opcode, cpu):
    ...
    original_value = cpu.V[opcode.x]
    result = cpu.V[opcode.x] + \
        cpu.V[opcode.y]
    ...
    cpu.V[opcode.x] = result
 
def take_x_from_y(opcode, cpu):
    cpu.set_arithmetic_flag()
 
    original_value = cpu.V[opcode.x]
    result = cpu.V[opcode.y] - \
        cpu.V[opcode.x]
...
def bitwise_and(opcode, cpu):
    cpu.V[opcode.x] = cpu.V[opcode.x] & cpu.V[opcode.y]
...
def shift_x_left(opcode, cpu):
    most_significant_bit = (cpu.V[opcode.x] >> 7)
    cpu.V[CARRY_FLAG_ADDRESS] = most_significant_bit
    # restrict the shifted value to one byte or less
    cpu.V[opcode.x] = (
        cpu.V[opcode.x] << 1) & 0xFF
 
def shift_x_right(opcode, cpu):
    least_significant_bit = cpu.V[opcode.x] & 0x01
    cpu.V[CARRY_FLAG_ADDRESS] = least_significant_bit
    # restrict the shifted value to one byte or less
    cpu.V[opcode.x] = cpu.V[opcode.x] >> 1
 
def clear_display(opcode, cpu):
    for x in range(cpu.FRAME_BUFFER_WIDTH):
        for y in range(cpu.FRAME_BUFFER_HEIGHT):
            cpu.screenbuffer[x][y] = bool()
...
def draw_sprite(opcode, cpu):
    ...
    for current_row_offset in range(height):
        ...
        new_pixels = cpu.ram[cpu.I + current_row_offset]
        ...
        for x_offset in range(8):
            ...
            old_bit = cpu.screenbuffer[column][row]
            new_bit = bool(new_pixels & mask)
            bit_value = old_bit ^ new_bit
            cpu.screenbuffer[column][row] = True if bit_value else False
...
def wait_for_key_press(opcode, cpu):
    if not cpu.keys:
        ...
        cpu.V[opcode.x] = sorted(cpu.keys)[0]
...
def goto_plus(opcode, cpu):
    cpu.pc = cpu.V[0] + opcode.nnn
    cpu.pc &= 0xFFFF  # restrict the PC value to two bytes or less
...
def generate_random(opcode, cpu):
    ...
    cpu.V[opcode.x] = opcode.nn & random_int
...
def save_x_as_bcd(opcode, cpu):
    value = cpu.V[opcode.x]
    # store the most significant digit as a byte
    cpu.ram[cpu.I] = int(value / 100) & 0xFF
    # store the middle digit as a byte
    cpu.ram[cpu.I + 1] = int((value / 10) % 10) & 0xFF
    # store the least significant digit as a byte
    cpu.ram[cpu.I + 2] = int(value % 10) & 0xFF
